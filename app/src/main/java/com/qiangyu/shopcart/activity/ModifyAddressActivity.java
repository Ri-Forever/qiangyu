...
public class ModifyAddressActivity extends Activity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mContacts = getIntent().getStringExtra("contacts");
        mMobile = getIntent().getStringExtra("mobile");
        mAddress = getIntent().getStringExtra("location");
        mAddress1 = getIntent().getStringExtra("address");
        mADefault = getIntent().getStringExtra("Default");
        mDizhiId = getIntent().getStringExtra("locationIds");
    ...
    private void initView() {
        ...
        if (StringUtils.isNotEmpty(mContacts)) {
            etShouhuoren.setText(mContacts);
        }
        if (StringUtils.isNotEmpty(mMobile)) {
            etDianhua.setText(mMobile);
        }
        if (StringUtils.isNotEmpty(mAddress)) {
            tvQuyu.setText(mAddress);
        }
        if (StringUtils.isNotEmpty(mAddress1)) {
            etDizhi.setText(mAddress1);
        }
        if (StringUtils.isNotEmpty(mADefault)) {
            if ("true".equals(mADefault)) {
        ...
        tvBaocun.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                getDataFromEditUserAddress(CityId, mMid, mId, shouhuoren, dianhua, mDizhiId, mAddress, dizhi, isDefault, mSignInPwd);
            }
        });
    }
    ...
    */
    ...
    private void EditUserAddress(String json) {
        ...
        if ("OK".equals(code)) {
            ToastUtil.toastCenter(mContext, "修改地址成功!");
            finish();
        } else {
            ToastUtil.toastCenter(mContext, "修改地址失败!");
        }
    }
}
